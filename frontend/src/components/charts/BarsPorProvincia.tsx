...
export default function BarsPorProvincia({ data, filtros }: Props) {
  const rows = filtrar(data, filtros);
  const counts = groupCount(rows, d => d.provincia);
  const chartData = toBarData(counts, 'nome', 'valor');

  return (
    <ChartCard title="Denúncias por Província" subtitle="Total de denúncias na plataforma">
